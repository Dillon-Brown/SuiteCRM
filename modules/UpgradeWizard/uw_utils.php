...
function commitCopyNewFiles($unzip_dir, $zip_from_dir, $path = '')
{
    ...
    foreach ($newFiles as $file) {
        ...
        if ((!file_exists($targetFile)) || /* brand new file */
            ...
            if (strpos($targetFile, 'sugar_version.php') !== false && !preg_match('/\/portal\/sugar_version\.php$/i',
                    $targetFile)
            ) {
    ...
    if (file_exists($from)) {
        ...
        foreach ($modifiedFiles as $file) {
            ...
            if (strpos($srcFile, ".svn") === false) {
                ...
                // handle sugar_version.php
                if (strpos($targetFile,
                        'sugar_version.php') !== false && !preg_match('/\/portal\/sugar_version\.php$/i', $targetFile)
                ) {
...
function checkSystemCompliance()
{
    ...
    if ($canInstall !== true) {
        ...
        if (count($canInstall) == 1) {
            ...
            $ret['dbVersion'] = "<b><span class=stop>" . sprintf($installer_mod_strings[$canInstall[0]],
                    $canInstall[1]) . "</span></b>";
        }
    }
...
function fileCopy($file_path)
{
    if (file_exists(clean_path($_SESSION['unzip_dir'] . '/' . $_SESSION['zip_from_dir'] . '/' . $file_path))) {
        ...
        $destFile = str_replace(clean_path($_SESSION['unzip_dir'] . '/' . $_SESSION['zip_from_dir']),
            clean_path(getcwd()), $file);
        if (!is_dir(dirname($destFile))) {
            mkdir_recursive(dirname($destFile)); // make sure the directory exists
        }
        copy_recursive($file, $destFile);
...
if (!function_exists('extractFile')) {
    function extractFile($zip_file, $file_in_zip)
    {
        ...
        $relative_base_tmp_upgrade_dir = clean_path(str_replace(clean_path(getcwd()), '',
...
function getImageForType($type)
{
    ...
    switch ($type) {
        case "full":
            $icon = SugarThemeRegistry::current()->getImage("Upgrade", "", null, null, '.gif',
                $mod_strings['LBL_UPGRADE']);
            break;
        case "langpack":
            $icon = SugarThemeRegistry::current()->getImage("LanguagePacks", "", null, null, '.gif',
                $mod_strings['LBL_LANGPACKS']);
            break;
        case "module":
            $icon = SugarThemeRegistry::current()->getImage("ModuleLoader", "", null, null, '.gif',
                $mod_strings['LBL_MODULELOADER']);
            break;
        case "patch":
            $icon = SugarThemeRegistry::current()->getImage("PatchUpgrades", "", null, null, '.gif',
                $mod_strings['LBL_PATCHUPGRADES']);
            break;
        case "theme":
            $icon = SugarThemeRegistry::current()->getImage("Themes", "", null, null, '.gif',
                $mod_strings['LBL_THEMES']);
            break;
        default:
            break;
    }
    ...
    foreach ($module_directories as $module_dir) {
        ...
        if (file_exists($read_searchdefs_from) && file_exists($read_SearchFields_from)) {
            ...
            if (!$found_sf1 && !$found_sf2) {
                $searchdefs[$module_dir]['layout']['basic_search']['favorites_only'] = array(
                    'name' => 'favorites_only',
                    'label' => 'LBL_FAVORITES_FILTER',
                    'type' => 'bool',
                );
                $searchdefs[$module_dir]['layout']['advanced_search']['favorites_only'] = array(
                    'name' => 'favorites_only',
                    'label' => 'LBL_FAVORITES_FILTER',
                    'type' => 'bool',
                );
                ...
                $success_sf1 = write_array_to_file('searchdefs', $searchdefs,
                    "custom/{$full_module_dir}/metadata/searchdefs.php");
                $success_sf2 = write_array_to_file('searchFields', $searchFields,
                    "{$full_module_dir}/metadata/SearchFields.php");
...
function upgradeTeamColumn($bean, $column_name)
{
    ...
    if (empty($bean->field_defs['team_set_id']) && $bean->module_dir != 'Trackers') {
        ...
        $relationshipDiff = array_diff_assoc($vardefs['relationships'],
    ...
    if (file_exists('custom/include/language')) {
        ...
        foreach ($files as $file) {
            ...
            if (isset($GLOBALS['app_list_strings']) && is_array($GLOBALS['app_list_strings'])) {
                ...
                if (!$touched) {
                    if (preg_match('/\$GLOBALS\s*\[\s*[\"|\']app_list_strings[\"|\']\s*\]\s*=\s*array\s*\(/',
                        $contents)) {
                        ...
                        if (preg_match('/language\/([^\.]+)\.lang\.php$/', $file, $matches)) {
                            ...
                            if (file_exists("include/language/$language.lang.override.php")) {
                                $app_list_strings = _mergeCustomAppListStrings("include/language/$language.lang.override.php",
                                    $app_list_strings);
                            }
                            if (file_exists("custom/application/Ext/Language/$language.ext.lang.php")) {
                                $app_list_strings = _mergeCustomAppListStrings("custom/application/Ext/Language/$language.ext.lang.php",
                                    $app_list_strings);
                            }
                            if (file_exists("custom/application/Ext/Language/$language.lang.ext.php")) {
                                $app_list_strings = _mergeCustomAppListStrings("custom/application/Ext/Language/$language.lang.ext.php",
                                    $app_list_strings);
                            }
                            ...
                            if (!empty($GLOBALS['app_list_strings'])) {
                                foreach ($GLOBALS['app_list_strings'] as $key => $value) {
                                    ...
                                    if (isset($all_non_custom_include_language_list_strings[$key])) {
                                        $diff = array_diff($all_non_custom_include_language_list_strings[$key],
                                            $GLOBALS['app_list_strings'][$key]);
                                    }
    ...
    while ($row = $db->fetchByAssoc($result)) {
        ...
        if (!empty($content['dashlets']) && !empty($content['pages'])) {
            ...
            foreach ($originalDashlets as $key => $ds) {
                if (!empty($ds['options']['url']) && stristr($ds['options']['url'],
                        'https://www.sugarcrm.com/crm/product/gopro')
                ) {
...
function upgradeDateTimeFields($path)
{
    ...
    $meetingsSql = "UPDATE meetings SET date_end = " . $db->convert("date_start", 'add_time',
            array('duration_hours', 'duration_minutes'));
...
)
{
    if (!empty($config_location) && !file_exists($config_location)) {
        if ($write_to_upgrade_log) {
            logThis('config.php file specified in ' . $config_si_location . ' could not be found.  Skip merging',
                $path);
        }
    ...
    if (!empty($config_si_location) && !file_exists($config_si_location)) {
        if ($write_to_upgrade_log) {
            logThis('config_si.php file specified in ' . $config_si_location . ' could not be found.  Skip merging',
                $path);
        }

        return false;
    } else {
        if (empty($config_si_location)) {
...
function repairSearchFields($globString = 'modules/*/metadata/SearchFields.php', $path = '')
{
    ...
    foreach ($searchFieldsFiles as $file) {
        if (preg_match('/modules\/(.*?)\/metadata\/SearchFields\.php/', $file,
                $matches) && isset($beanList[$matches[1]])
        ) {
            $module = $matches[1];
