...
eof;
    ...
    public function handleAttachmentsProcessImages(
        $focus,
        $redirect,
        $useSiteURL = false,
        $entryPoint = 'download',
    ...
    public function processImages(&$focus, $useSiteURL, $entryPoint, $useUploadFolder)
    {
        ...
        if (strpos($emailTemplateBodyHtml, '"cache/images/')) {
            $matches = array();
            preg_match_all('#<img[^>]*[\s]+src[^=]*=[\s]*["\']cache/images/(.+?)["\']#si', $emailTemplateBodyHtml,
                $matches);
            foreach ($matches[1] as $match) {
                ...
                if (file_exists($file_location)) {
                    ...
                    if (!copy($file_location, $newFileLocation)) {
                        ...
                        $emailTemplateBodyHtml = str_replace("cache/images/$match", $secureLink,
