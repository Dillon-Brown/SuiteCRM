    ...
    public function authenticateUser($name, $password, $fallback = false)
    {
        ...
        if ($bind) {
            ...
            if (!empty($GLOBALS['ldap_config']->settings['ldap_group'])
                ...
                && !empty($GLOBALS['ldap_config']->settings['ldap_group_attr']) && !in_array($attrs,
                    $GLOBALS['ldap_config']->settings['ldap_group_user_attr'], true)) {
