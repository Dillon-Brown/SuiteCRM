...
class CalendarController extends SugarController
{
    ...
    protected function action_saveactivity()
    {
        ...
        if (!file_exists($path)) {
            LoggerManager::getLogger()->fatal("File {$bean->object_name}FormBase.php doesn't exist");
            sugar_cleanup(true);
        }
        ...
        if (!class_exists($FBObjectName)) {
            LoggerManager::getLogger()->fatal("Class {$bean->object_name}FormBase doesn't exist");
            sugar_cleanup(true);
        }
