    ...
    function sendEmail($mail, $save_emails = 1, $testmode = false)
    {
        ...
        if ((!isset($module->email_opt_out)
            ...
            if (empty($this->current_mailbox->id) or $this->current_mailbox->id !== $this->current_emailmarketing->inbound_email_id) {
                ...
                $this->mailbox_from_addr = $this->current_mailbox->get_stored_options('from_addr', 'nobody@example.com',
                    null);
            }
            ...
            $mail->FromName = $locale->translateCharsetMIME(trim($this->current_emailmarketing->from_name), 'UTF-8',
                $OBCharset);
            ...
            $replyToName = empty($this->current_emailmarketing->reply_to_name) ? $this->current_mailbox->get_stored_options('reply_to_name',
                $mail->FromName, null) : $this->current_emailmarketing->reply_to_name;
            $replyToAddr = empty($this->current_emailmarketing->reply_to_addr) ? $this->current_mailbox->get_stored_options('reply_to_addr',
                $mail->From, null) : $this->current_emailmarketing->reply_to_addr;
            ...
            $template_data = $this->current_emailtemplate->parse_email_template(array(
            ...
            $template_data = $this->current_emailtemplate->parse_tracker_urls($template_data, $tracker_url_template,
                $this->tracker_urls, $removeme_url_template);
            ...
            if ($text_only) {
                ...
                if ($btracker) {
                    ...
                    if (!empty($tracker_url)) {
                        ...
                        $mail->AltBody = str_replace('TRACKER_URL_START', "<a href='" . $tracker_url . "'>",
                            $mail->AltBody);
