...
//create the html to create tracker table
    foreach ($trkr_lists as $trkr_id) {
        ...
        if (isset($ct_focus->tracker_name) && !empty($ct_focus->tracker_name)) {
            ...
            $trkr_html .= SugarThemeRegistry::current()->getImage('delete_inline', "border='0'  align='absmiddle'", 12,
                    12, ".gif", $mod_strings['LBL_DELETE']) . "</a></td></tr></table></div>";
...
//if prospect lists are returned, then iterate through and populate form values
    ...
    if (count($prospect_lists) > 0) {
        ...
        foreach ($prospect_lists as $pl_id) {
            ...
            $trgt_html .= SugarThemeRegistry::current()->getImage('delete_inline', "border='0' align='absmiddle'", 12,
                    12, ".gif", $mod_strings['LBL_DELETE']) . "</a></td></tr></table></div>";
...
if ($campaign_type == 'general') {
    ...
    $ss->assign('NAV_ITEMS',
        create_wiz_menu_items($_steps, 'email', $mrkt_string, $summ_url, 'dotlist', $campaign_id, $marketing_id,
            $template_id));
...
function create_wiz_menu_items(
    $steps,
    $type,
    $mrkt_string,
    $summ_url,
