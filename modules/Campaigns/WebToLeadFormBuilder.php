            ...
            if ($colsFieldCount = count($formCols)) {
                ...
                for ($j = 0; $j < $colsFieldCount; $j++) {
                    ...
                    if (isset($lead->field_defs[$colsFields[$j]]) && $lead->field_defs[$colsFields[$j]] != null) {
                        ...
                        list($field_name, $field_label, $field_type, $field_required, $field_options) = self::getArrayOfFieldInfo($lead,
                            $colsFields[$j], $required_fields);

                        if ($field_type == 'multienum' || $field_type == 'enum' || $field_type == 'radioenum') {
                            $colHtml .= self::getFieldEnumHTML($lead, $field_name, $appListStrings[$field_options],
                                $field_required, $field_label, $webRequiredSymbol, $colsFields[$j]);
                            $foundField = true;
                        } elseif ($field_type == 'bool') {
                            $colHtml .= self::getFieldBoolHTML($field_name, $field_required, $field_label,
                                $webRequiredSymbol);
                            $foundField = true;
                            ...
                            $colHtml .= self::getFieldDateHTML($field_name, $field_required, $field_label,
                                $webRequiredSymbol);
                            $foundField = true;
                        } elseif ($field_type == 'varchar' || $field_type == 'name' || $field_type == 'phone' || $field_type == 'currency' || $field_type == 'url' || $field_type == 'int') {
                            $colHtml .= self::getFieldCharsHTML($field_name, $field_label, $field_required,
                                $webRequiredSymbol);
                            $foundField = true;
                        } elseif ($field_type == 'text') {
                            $colHtml .= self::getFieldTextHTML($field_name, $field_label, $field_required && false,
                                $webRequiredSymbol);
                            $foundField = true;
                        } elseif ($field_type == 'relate' && $field_name == 'account_name') {
                            $colHtml .= self::getFieldRelateHTML($field_name, $field_label, $field_required && false,
                                $webRequiredSymbol);
                            $foundField = true;
...
html;
    ...
    private static function getFieldEnumHTML(
        $lead,
        $fieldName,
        ...
        $leadOptions = get_select_options_with_id($appListStringsFieldOptions,
            !empty($lead->$fieldName) ? unencodeMultienum($lead->$fieldName) : '');
    ...
    private static function getFormFooterHTML(
        $webFormFooter,
        $webFormSubmitLabel,
        $webFormCampaign,
        $webRedirectURL,
