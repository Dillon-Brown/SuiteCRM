...
if (isset($_REQUEST['run']) && ($_REQUEST['run'] != "")) {
    ...
    if ($run == "upload") {
        ...
        if (isset($_REQUEST['release_id']) && $_REQUEST['release_id'] != "") {
            ...
            copy($_REQUEST['load_module_from_dir'] . '/' . $_REQUEST['upgrade_zip_escaped'],
                "upload://" . $_REQUEST['upgrade_zip_escaped']);
        ...
        if ($perform) {
            ...
            if (is_file($manifest_file)) {
                ...
                if (isset($manifest['icon']) && $manifest['icon'] != "") {
                    ...
                    copy($icon_location,
                        remove_file_extension($target_path) . "-icon." . pathinfo($icon_location, PATHINFO_EXTENSION));
                }
...
if ($view == "module") {
    print(getClassicModuleTitle($mod_strings['LBL_MODULE_NAME'], array($mod_strings['LBL_MODULE_LOADER_TITLE']),
        false));
} else {
    print(getClassicModuleTitle($mod_strings['LBL_MODULE_NAME'],
        array($mod_strings['LBL_MODULE_NAME'], $mod_strings['LBL_UPGRADE_WIZARD_TITLE']), false));
}
