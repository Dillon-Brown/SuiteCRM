    ...
    protected function importRow($row)
    {
        ...
        for ($fieldNum = 0; $fieldNum < $_REQUEST['columncount']; $fieldNum++) {
            ...
            if (isset($row[$fieldNum])) {
                ...
                $rowValue = $locale->translateCharset(strip_tags(trim($row[$this->sugarToExternalSourceFieldMap[$field]])),
                    $this->importSource->importlocale_charset, $sugar_config['default_charset']);
            } else {
                $rowValue = '';
            }
