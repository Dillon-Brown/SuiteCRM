...
class ModuleInstaller
{
    ...
    public function install_copy()
    {
        if (isset($this->installdefs['copy'])) {
            ...
            foreach ($this->installdefs['copy'] as $cp) {
                LoggerManager::getLogger()->debug("Copying ..." . $cp['from'].  " to " .$cp['to']);
                /* BEGIN - RESTORE POINT - by MR. MILK August 31, 2005 02:22:11 PM */
    ...
    public function uninstall_copy()
    {
        if (!empty($this->installdefs['copy'])) {
            foreach ($this->installdefs['copy'] as $cp) {
                ...
                LoggerManager::getLogger()->debug('Unlink ' . $cp['to']);
                /* BEGIN - RESTORE POINT - by MR. MILK August 31, 2005 02:22:11 PM */
    ...
    public function uninstall_new_files($cp, $backup_path)
    {
        ...
        foreach ($zip_files as $k=>$v) {
            ...
            if (!isset($backup_files[$k])) {
                ...
                if (!$this->ms->sugarFileExists($to)) {
                    LoggerManager::getLogger()->debug('ModuleInstaller[uninstall_new_file] deleting file ' . $to);
                    if (file_exists($to)) {
                        unlink($to);
                    }
                } else {
                    LoggerManager::getLogger()->fatal('ModuleInstaller[uninstall_new_file] Could not remove file ' . $to . ' as no backup file was found to restore to');
                }
            }
        }
        ...
        if (file_exists($cp['to']) && $files_remaining == 0) {
            LoggerManager::getLogger()->debug('ModuleInstaller[uninstall_new_file] deleting directory ' . $cp['to']);
            rmdir_recursive($cp['to']);
        }
    }
    ...
    public function installExt($section, $extname, $module = '')
    {
        if (isset($this->installdefs[$section])) {
            ...
            foreach ($this->installdefs[$section] as $item) {
                ...
                LoggerManager::getLogger()->debug("Installing section $section from $from for " .$item['to_module']);
    ...
    public function uninstallExt($section, $extname, $module = '')
    {
        if (isset($this->installdefs[$section])) {
            ...
            foreach ($this->installdefs[$section] as $item) {
                ...
                LoggerManager::getLogger()->debug("Uninstalling section $section from $from for " .$item['to_module']);
                if ($item['to_module'] == 'application') {
                    $path = "custom/Extension/application/Ext/$extname";
                } else {
                    $path = "custom/Extension/modules/{$item['to_module']}/Ext/$extname";
    ...
    public function disableExt($section, $extname, $module = '')
    {
        if (isset($this->installdefs[$section])) {
            foreach ($this->installdefs[$section] as $item) {
                ...
                LoggerManager::getLogger()->debug("Disabling $extname ... from $from for " .$item['to_module']);
                if ($item['to_module'] == 'application') {
                    $path = "custom/Extension/application/Ext/$extname";
                } else {
                    $path = "custom/Extension/modules/{$item['to_module']}/Ext/$extname";
    ...
    public function enableExt($section, $extname, $module = '')
    {
        if (isset($this->installdefs[$section])) {
            foreach ($this->installdefs[$section] as $item) {
                ...
                LoggerManager::getLogger()->debug("Enabling $extname ... from $from for " .$item['to_module']);
    ...
    public function install_dashlets()
    {
        if (isset($this->installdefs['dashlets'])) {
            foreach ($this->installdefs['dashlets'] as $cp) {
                ...
                LoggerManager::getLogger()->debug("Installing Dashlet " . $cp['name'] . "..." . $cp['from']);
                if (!file_exists($path)) {
                    mkdir_recursive($path, true);
                }
    ...
    public function uninstall_dashlets()
    {
        if (isset($this->installdefs['dashlets'])) {
            foreach ($this->installdefs['dashlets'] as $cp) {
                ...
                $path = 'custom/modules/Home/Dashlets/' . $cp['name'];
                LoggerManager::getLogger()->debug('Unlink ' .$path);
                if (file_exists($path)) {
    ...
    public function install_dcactions()
    {
        if (isset($this->installdefs['dcaction'])) {
            ...
            foreach ($this->installdefs['dcaction'] as $action) {
                ...
                LoggerManager::getLogger()->debug("Installing DCActions ..." . $action['from']);
    ...
    public function uninstall_dcactions()
    {
        if (isset($this->installdefs['dcaction'])) {
            ...
            foreach ($this->installdefs['dcaction'] as $action) {
                $action['from'] = str_replace('<basepath>', $this->base_dir, $action['from']);
                LoggerManager::getLogger()->debug("Uninstalling DCActions ..." . $action['from']);
                $path = 'custom/Extension/application/Ext/DashletContainer/Containers';
    ...
    public function install_connectors()
    {
        if (isset($this->installdefs['connectors'])) {
            foreach ($this->installdefs['connectors'] as $cp) {
                ...
                LoggerManager::getLogger()->debug("Installing Connector " . $cp['name'] . "..." . $cp['connector']);
    ...
    public function uninstall_connectors()
    {
        if (isset($this->installdefs['connectors'])) {
            foreach ($this->installdefs['connectors'] as $cp) {
                ...
                $source_path = 'custom/modules/Connectors/connectors/sources/' . $dir;
                $formatter_path = 'custom/modules/Connectors/connectors/formatters/' . $dir;
                LoggerManager::getLogger()->debug('Unlink ' .$source_path);
    ...
    {
        LoggerManager::getLogger()->debug("Installing Vardefs ..." . $from .  " for " .$to_module);
    ...
    {
        LoggerManager::getLogger()->debug("Installing Layout Defs ..." . $from .  " for " .$to_module);
    ...
    public function install_languages()
    {
        ...
        if (isset($this->installdefs['language'])) {
            ...
            foreach ($this->installdefs['language'] as $packs) {
                ...
                LoggerManager::getLogger()->debug("Installing Language Pack ..." . $packs['from']  .  " for " .$packs['to_module']);
    ...
    public function uninstall_languages()
    {
        ...
        if (isset($this->installdefs['language'])) {
            ...
            foreach ($this->installdefs['language'] as $packs) {
                ...
                $languages[$packs['language']] = $packs['language'];
                $packs['from'] = str_replace('<basepath>', $this->base_dir, $packs['from']);
                LoggerManager::getLogger()->debug("Uninstalling Language Pack ..." . $packs['from']  .  " for " .$packs['to_module']);
                $path = 'custom/Extension/modules/' . $packs['to_module']. '/Ext/Language';
                if ($packs['to_module'] == 'application') {
                    $path ='custom/Extension/' . $packs['to_module']. '/Ext/Language';
                }
    ...
    public function disable_languages()
    {
        if (isset($this->installdefs['language'])) {
            ...
            foreach ($this->installdefs['language'] as $item) {
                $from = str_replace('<basepath>', $this->base_dir, $item['from']);
                LoggerManager::getLogger()->debug("Disabling Language {$item['language']}... from $from for " .$item['to_module']);
    ...
    public function enable_languages()
    {
        if (isset($this->installdefs['language'])) {
            foreach ($this->installdefs['language'] as $item) {
                $from = str_replace('<basepath>', $this->base_dir, $item['from']);
                LoggerManager::getLogger()->debug("Enabling Language {$item['language']}... from $from for " .$item['to_module']);
                $modules[]=$item['to_module'];
                $languages[$item['language']] = $item['language'];
                if (!empty($module)) {
                    $item['to_module'] = $module;
    ...
    public function copy_path($from, $to, $backup_path='', $uninstall=false)
    {
        ...
        if (!$uninstall) {
            ...
            LoggerManager::getLogger()->debug('Copy ' . $from);
        } else {
            $from = str_replace('<basepath>', $backup_path, $from);
            //LoggerManager::getLogger()->debug('Restore ' . $from);
        }
    ...
    public function install_custom_fields($fields)
    {
        ...
        foreach ($fields as $field) {
            ...
            if (!$installed) {
                LoggerManager::getLogger()->debug('Could not install custom field ' . $field['name'] . ' for module ' .  $field['module'] . ': Module does not exist');
            }
        }
    }
    ...
    public function install_relationship($file)
    {
        ...
        if (!file_exists($file)) {
            LoggerManager::getLogger()->debug('File does not exists : '.$file);
            return;
        }
        include($file);
        ...
        foreach ($rel_dictionary as $rel_name => $rel_data) {
            ...
            if (!$this->silent) {
                LoggerManager::getLogger()->debug("Processing relationship meta for ". $rel_name."...");
            }
            SugarBean::createRelationshipMeta($rel_name, $this->db, $table, $rel_dictionary, '');
            Relationship::delete_cache();
            if (!$this->silent) {
                LoggerManager::getLogger()->debug('done<br>');
            }
        }
    }
    ...
    {
        if (!$application) {
            LoggerManager::getLogger()->debug(get_class($this)."->merge_files() : merging module files in custom/Extension/modules/<module>/$path to custom/modules/<module>/$path$name");
            foreach ($this->modules as $module) {
                //LoggerManager::getLogger()->debug("Merging Files for: ".$module);
                //LoggerManager::getLogger()->debug("Merging Files for path: ".$path);
                $extension = "<?php \n //WARNING: The contents of this file are auto-generated\n";
            ...
            if (isset($beanList[$bean])) {
                ...
                if (file_exists($beanFiles[$class])) {
                    ...
                    if (is_subclass_of($mod, 'SugarBean')  && $mod->disable_vardefs == false) {
                        LoggerManager::getLogger()->debug("Creating Tables Bean : $bean");
                        $mod->create_tables();
                        SugarBean::createRelationshipMeta($mod->getObjectName(), $mod->db, $mod->table_name, array(), $mod->module_dir);
                    }
                } else {
                    LoggerManager::getLogger()->debug("File Does Not Exist:" . $beanFiles[$class]);
                }
            }
        }
    }
            ...
            if (isset($beanList[$bean])) {
                ...
                if (file_exists($beanFiles[$class])) {
                    ...
                    if (is_subclass_of($mod, 'SugarBean')) {
                        LoggerManager::getLogger()->debug("Drop Tables : $bean");
                        if (isset($GLOBALS['mi_remove_tables']) && $GLOBALS['mi_remove_tables']) {
                            $mod->drop_tables();
                        }
                    }
    ...
    {
        LoggerManager::getLogger()->debug('ModuleInstaller:'. $str);
        if (!$this->silent) {
            echo $str . '<br>';
        }
    }
    ...
    public function copy_recursive_with_backup($source, $dest, $backup_path, $uninstall=false)
    {
        if (is_file($source)) {
            if ($uninstall) {
                LoggerManager::getLogger()->debug("Restoring ... " . $source.  " to " .$dest);
                if (copy($source, $dest)) {
                    if (is_writable($dest)) {
                ...
                LoggerManager::getLogger()->debug("Can't restore file: " . $source);
                return true;
            }
            if (file_exists($dest)) {
                ...
                LoggerManager::getLogger()->debug("Backup ... " . $dest.  " to " .$rest);
                if (copy($dest, $rest)) {
                    if (is_writable($rest)) {
                        sugar_touch($rest, filemtime($dest));
                    }
                } else {
                    LoggerManager::getLogger()->debug("Can't backup file: " . $dest);
                }
            }
    ...
    public function enable($base_dir, $is_upgrade = false, $previous_version = '')
    {
        ...
        if (file_exists($this->base_dir . '/manifest.php')) {
            ...
            LoggerManager::getLogger()->debug('Complete');
        } else {
            die("No \$installdefs Defined In $this->base_dir/manifest.php");
        }
    }
    ...
    public function enable_dashlets()
    {
        if (isset($this->installdefs['dashlets'])) {
            foreach ($this->installdefs['dashlets'] as $cp) {
                ...
                LoggerManager::getLogger()->debug("Enabling Dashlet " . $cp['name'] . "..." . $cp['from']);
    ...
    public function disable_dashlets()
    {
        if (isset($this->installdefs['dashlets'])) {
            foreach ($this->installdefs['dashlets'] as $cp) {
                $path = 'custom/modules/Home/Dashlets/' . $cp['name'];
                $disabled_path = 'custom/modules/Home/'.DISABLED_PATH.'Dashlets/' . $cp['name'];
                LoggerManager::getLogger()->debug('Disabling ' .$path);
                if (file_exists($path)) {
            ...
            if (!empty($this->installdefs['copy'])) {
                foreach ($this->installdefs['copy'] as $cp) {
                    ...
                    if (file_exists($backup_path)) {
                        ...
                        LoggerManager::getLogger()->debug("ENABLE COPY:: FROM: ".$cp['from']. " TO: ".$cp['to']);
                        $this->copy_path($cp['from'], $cp['to']);
                        /*}else{
    ...
    public function disable_copy()
    {
        ...
        //		LoggerManager::getLogger()->debug('ModuleInstaller.php->disable_copy()');
        if (isset($GLOBALS['mi_overwrite_files']) && $GLOBALS['mi_overwrite_files']) {
            //		LoggerManager::getLogger()->debug('ModuleInstaller.php->disable_copy():mi_overwrite_files=true');
            if (!empty($this->installdefs['copy'])) {
                //				LoggerManager::getLogger()->debug('ModuleInstaller.php->disable_copy(): installdefs not empty');
                foreach ($this->installdefs['copy'] as $cp) {
                    $cp['to'] = clean_path(str_replace('<basepath>', $this->base_dir, $cp['to']));
                    ...
                    //check if this file exists in the -restore directory
                    //					LoggerManager::getLogger()->debug("ModuleInstaller.php->disable_copy(): backup_path=".$backup_path);
                    if (file_exists($backup_path)) {
                        //since the file exists, then we want do an md5 of the install version and the file system version
                        ...
                        //since the files are the same then we can safely move back from the -restore
                        //directory into the file system
                        LoggerManager::getLogger()->debug("DISABLE COPY:: FROM: ".$backup_path. " TO: ".$cp['to']);
