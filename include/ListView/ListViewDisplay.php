...
class ListViewDisplay
{
    ...
    function setup(
        ...
        $data = $this->lvd->getListViewData($seed, $where, $offset, $limit, $filter_fields, $params, $id_field, true,
            $id);
    ...
    public function display()
    {
        ...
        if ($this->show_mass_update_form) {
            $str = $this->mass->getDisplayMassUpdateForm(true,
                    $this->multi_select_popup) . $this->mass->getMassUpdateFormHeader($this->multi_select_popup);
        }
    ...
    protected function buildActionsLink($id = 'actions_link', $location = 'top')
    {
        ...
        if (isset($this->templateMeta['form']['actions'])) {
            ...
            if ((ACLController::checkAccess($this->seed->module_dir, 'edit',
                        true) && ACLController::checkAccess($this->seed->module_dir, 'massupdate',
                        true)) && $this->showMassupdateFields && $mass->doMassUpdateFieldsExistForFocus()
            ) {
