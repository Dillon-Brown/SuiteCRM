    ...
    */
    public function processListNavigation($xtemplateSection, $html_varName, $current_offset, $next_offset, $previous_offset, $row_count, $sugarbean=null, $subpanel_def=null, $col_count = 20)
    {
        ...
        if (empty($this->query_limit)  || $this->query_limit > $this->records_per_page) {
            ...
            $moduleString = htmlspecialchars("{$currentModule}_{$html_varName}_offset", ENT_QUOTES | ENT_HTML5);
            $moduleStringOrder = htmlspecialchars("{$currentModule}_{$html_varName}_ORDER_BY", ENT_QUOTES | ENT_HTML5);
            if ($this->shouldProcess && !$this->multi_select_popup) {
                ...
                $select_entire_list = ($massUpdateRun) ? 0 : (isset($_POST['select_entire_list']) ? $_POST['select_entire_list'] : (isset($_REQUEST['select_entire_list']) ? htmlspecialchars($_REQUEST['select_entire_list'],
