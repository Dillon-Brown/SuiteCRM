...
//        $this->checkConnection();
        $db = new PDO('mysql:host=mariadb;dbname=suitecrm-mysql', 'root', 'ChangeMe2');
    ...
    {
        return new PDO('mysql:host=mariadb;dbname=suitecrm-mysql', 'root', 'ChangeMe2');
    }
    ...
    protected function keysSQL($indices, $alter_table = false, $alter_action = '')
    {
        ...
        foreach ($indices as $index) {
            if (!empty($index['db']) && $index['db'] != $sugar_config['dbconfig']['db_type']) {
                continue;
            }
